...
export default class MongoSensorStorage extends SensorStorage
{
    ...
    async _findAndUpdateValue(time, rawValue)   //查找并更新值
    {
     ...
     const seconds = minutes * 60 + time.getSeconds();
     ...
     else if (this.sensor.config.monitor.interval === 1000)
     {
         if (JSON.stringify(doc.values[seconds]) !== JSON.stringify(blankValue))
         {
             logger.warn(`- [${this.sensor.name}] Duplicated update found. This update will be ignored.`);
             return;
         }
         else{
             set[`values.${seconds}`] = value;
         }
     }
...
/*         const set = {};
    ...
    _createBlankValues()
    {
        if (this.sensor.config.monitor.interval === 60 * 1000 || this.sensor.config.monitor.interval === 1000)
        {
            const values = [];
