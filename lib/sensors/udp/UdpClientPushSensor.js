...
export default class UdpClientPushSensor extends Sensor
{
    ...
    initMonitor()
    {
        serverSocket.on(`${this.id}:data`, function(e){
            let values = this.format(this.id, e.msg);
            console.log(`sensor-${this.id} recieved data: ${e.msg}, ${JSON.stringify(values)}`);
    ...
    format(sensorId, msg)
    {
        ...
        if (sensorId === '01')
        {
            ...
            values = {"x": parseFloat(result[2]), "y": parseFloat(result[3]), "z": parseFloat(result[4])}
        }
        return values;
    }
}
